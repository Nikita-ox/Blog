...
class PostListView(ListView):
    ...
    paginate_by = 3
    template_name = 'database_where/post/list.html'
...
def post_detail(request, post, year, month, day):
    ...
    comments = post.комментарии.filter(active=True)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = f"Read {post.title} at {post_url}\n\n" \
                      f"{cd['Имя']}\'комментарий: {cd['Комментарии']}"
            send_mail(subject, message, 'modaravvv@gmail.com',
                      [cd['Адрес_получателя']])
            sent = True
    else:
        form = EmailPostForm()
    return render(request, 'database_where/post/share.html', {'post': post,
                                                              'form': form,
                                                              'sent': sent})
...
def post_comment(request, post_id):
    ...
    if form.is_valid():
        ...
        comment.save()
    return render(request, 'database_where/post/comment.html',
                  {'post': post,
                   'form': form,
                   'comment': comment})
